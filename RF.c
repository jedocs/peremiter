...
void rf12_onOff(unsigned char value);
//BOOL hasPacket;
...
void RF_init(unsigned char nodeid, unsigned char freqBand, unsigned char groupid, unsigned char txPower, unsigned char airKbps) {
    ...
    /*
       ...
       // send_cmd(0xC691);         //Air transmission baud rate: 0x08= ~38.31Kbps
    ...
    send_cmd(0x8007 | (freqBand << 4)); // EL (ena TX), EF (ena RX FIFO), 12.0pF
    send_cmd(0x82c8);
    send_cmd(0xA000 | 3900); //640); // Frequency is exactly 434/868/915MHz (whatever freqBand is)
    // send_cmd(0xC691); //Air transmission baud rate: 0x08= ~38.31Kbps
    send_cmd(0x9080); //
    send_cmd(0xC23c); // AL,!ml,DIG,DQD4
    //send_cmd(0xCA83);
    //send_cmd(0xC483); // @PWR,NO RSTRIC,!st,!fi,OE,EN
    send_cmd(0x9871);//9870);
    send_cmd(0xCC77); // OB1, OB0, LPX, ddy, DDIT, BW0
    send_cmd(RF_XMITTER_ON);
...
void rf12_onOff(unsigned char value) {
    send_cmd(value ? RF_XMITTER_ON : RF_IDLE_MODE);
}
